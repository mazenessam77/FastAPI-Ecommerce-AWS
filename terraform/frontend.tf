###############################################################################
# Frontend Infrastructure — ECS Fargate for Next.js SSR
# Deployed behind an ALB, fronted by CloudFront for caching
###############################################################################
...
resource "aws_ecs_task_definition" "frontend" {
  ...
  container_definitions = jsonencode([{
    ...
    environment = [
      { name = "NEXT_PUBLIC_API_URL", value = aws_apigatewayv2_api.main.api_endpoint },
...
# NOTE: CloudFront disabled — account requires AWS verification.
# Frontend is accessed directly via ALB DNS.
# Re-enable CloudFront once account is verified for CDN caching.
